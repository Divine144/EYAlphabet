...
public class StormFollowGoal extends Goal {
    ...
    public StormFollowGoal(Mob pTamable, double pSpeedModifier, float pStartDistance, float pStopDistance, boolean pCanFly) {
        this.mob = pTamable;
    ...
    public boolean canUse() {
        UUID firelightId = GlobalCapabilityAttacher.getGlobalLevelCapability(this.mob.level).map(GlobalCapability::getFireLightUUID).orElse(null);
        if (firelightId == null)
            return false;

        LivingEntity livingentity = this.mob.level.getPlayerByUUID(firelightId);
